...
- adjacent context candidates near the plan with `path`, `role`,
  `selection_reason` и `read_by_default`: `state.md`, `current-state`, parent
  `README.md`, parent `plan.md`, `decisions.md`, если они существуют
...
- отметить `read_by_default: true` только для ближайшего scope guide и root guide;
  остальные candidates остаются hints, которые Codex читает по необходимости
